...
async def list_tools():
    return [
        Tool(name="get_all_loans",       description="Get all loan applications in the system.",
             inputSchema={"type":"object","properties":{},"required":[]}),
        Tool(name="get_loan_by_id",      description="Get a specific loan application by ID.",
             inputSchema={"type":"object","properties":{"loan_id":{"type":"integer"}},"required":["loan_id"]}),
        Tool(name="get_loans_by_customer",description="Get all loan applications for a customer.",
             inputSchema={"type":"object","properties":{"customer_id":{"type":"integer"}},"required":["customer_id"]}),
        Tool(name="get_loans_by_status", description="Get loans filtered by status (DRAFT/SUBMITTED/UNDER_REVIEW/APPROVED/REJECTED/DISBURSED).",
             inputSchema={"type":"object","properties":{"status":{"type":"string"}},"required":["status"]}),
        Tool(name="create_loan",         description="Create a new loan application.",
             inputSchema={"type":"object","properties":{
                 "customer_id":{"type":"integer"},"loan_type":{"type":"string"},
                 "amount":{"type":"number"},"purpose":{"type":"string"},
                 "tenure_months":{"type":"integer"},"priority":{"type":"string"}},
             "required":["customer_id","loan_type","amount","purpose"]}),
        Tool(name="submit_loan",         description="Submit a loan application for review.",
             inputSchema={"type":"object","properties":{"loan_id":{"type":"integer"}},"required":["loan_id"]}),
        Tool(name="approve_loan",        description="Approve a loan application.",
             inputSchema={"type":"object","properties":{
                 "loan_id":{"type":"integer"},"approver":{"type":"string"},"notes":{"type":"string"}},
             "required":["loan_id","approver"]}),
        Tool(name="reject_loan",         description="Reject a loan application with a reason.",
             inputSchema={"type":"object","properties":{
                 "loan_id":{"type":"integer"},"reason":{"type":"string"}},
             "required":["loan_id","reason"]}),
        Tool(name="disburse_loan",       description="Mark a loan as disbursed.",
             inputSchema={"type":"object","properties":{"loan_id":{"type":"integer"}},"required":["loan_id"]}),
    ]
    ...
    async with httpx.AsyncClient() as c:
        try:
            if   name == "get_all_loans":
                ...
                payload = {"customerId":arguments["customer_id"],"loanType":arguments["loan_type"],
                           "amount":arguments["amount"],"purpose":arguments["purpose"],
                           "tenureMonths":arguments.get("tenure_months",36),
                           "priority":arguments.get("priority","MEDIUM"),"currency":"USD"}
            ...
            elif name == "disburse_loan":
                r = await c.put(f"{BASE_URL}/api/v1/loans/{arguments['loan_id']}/disburse", timeout=30)
            else:
